...
sealed abstract class SyncIO[+A] private () {
   ...
   *        be released
   *
   * @return `SyncIO` produced by applying `use` to the result of the current `SyncIO`
   */
  def bracket[B](use: A => SyncIO[B])(release: A => SyncIO[Unit]): SyncIO[B] =
    flatMap { a =>
      (use(a).attempt <* release(a)).flatMap {
        case Left(t) => SyncIO.raiseError(t)
        case Right(b) => SyncIO.pure(b)
      }
    }
   ...
   * via `guarantee` in polymorphic code. Prefer `bracket`
   * for the acquisition and release of resources.
   *
   * @see [[SyncIO#bracket]] for the more general operation
   *
   * @param finalizer `SyncIO` to be executed when this `SyncIO` is finished
   * @return this `SyncIO` with registered `finalizer`
   */
  def guarantee(finalizer: SyncIO[Unit]): SyncIO[A] =
    SyncIO.unit.bracket(_ => this)(_ => finalizer)
  ...
  /**
   * Transforms this `SyncIO` into an [[IO]].
   *
   * @note This is a dangerous operation as it might end up executing blocking code on execution contexts which is not
   *       designated for that purpose.
   *
   * @return an `IO` that evaluates to the result of this `SyncIO`
   */
  def toIO: IO[A] =
    IO(unsafeRunSync())
  ...
  // mutable nullable state to avoid boxing of the result
  private[this] var resultFailure: Throwable = _
  private[this] var resultSuccess: A = _
  ...
  def unsafeRunSync(): A = {
    ...
    def runLoop(cur0: SyncIO[Any]): A = {
      if (cur0 == null) {
        if (resultFailure == null) {
          ...
          resultSuccess = null.asInstanceOf[A]
          return result
        } else {
          val up = resultFailure
          resultFailure = null
          throw up
        }
      }
    ...
    def runTerminusSuccessK(result: Any): SyncIO[Any] = {
      resultSuccess = result.asInstanceOf[A]
      null
    }

    def runTerminusFailureK(t: Throwable): SyncIO[Any] = {
      resultFailure = t
      null
    }
...
object SyncIO extends SyncIOLowPriorityImplicits {
   ...
   * @see [[SyncIO#to]]
   */
  def toK[F[_]: SyncEffect]: SyncIO ~> F =
    new (SyncIO ~> F) {
  ...
  implicit def showForSyncIO[A](implicit A: Show[A]): Show[SyncIO[A]] =
    Show.show { ioa =>
      (ioa.tag: @switch) match {
        case 0 =>
          val cur = ioa.asInstanceOf[Pure[A]]
          s"SyncIO(${A.show(cur.value)})"
        case _ => "IO(...)"
      }
    }
