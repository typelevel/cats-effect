...
private[unsafe] abstract class IORuntimeConfigCompanionPlatform { this: IORuntimeConfig.type =>
  ...
  protected final val Default: IORuntimeConfig = {
    val cancelationCheckThreshold =
      Option(System.getenv("CATS_EFFECT_CANCELATION_CHECK_THRESHOLD"))
        .flatMap(x => Try(x.toInt).toOption)
        .getOrElse(512)

    val autoYieldThreshold =
      Option(System.getenv("CATS_EFFECT_AUTO_YIELD_THRESHOLD_MULTIPLIER"))
        .flatMap(x => Try(x.toInt).toOption)
        .getOrElse(2) * cancelationCheckThreshold

    val enhancedExceptions = Option(System.getenv("CATS_EFFECT_TRACING_EXCEPTIONS_ENHANCED"))
      .flatMap(x => Try(x.toBoolean).toOption)
      .getOrElse(DefaultEnhancedExceptions)

    val traceBufferSize = Option(System.getenv("CATS_EFFECT_TRACING_BUFFER_SIZE"))
      .flatMap(x => Try(x.toInt).toOption)
      .getOrElse(DefaultTraceBufferSize)

    val shutdownHookTimeout = Option(System.getenv("CATS_EFFECT_SHUTDOWN_HOOK_TIMEOUT"))
      .flatMap(x => Try(Duration(x)).toOption)
      .getOrElse(DefaultShutdownHookTimeout)

    val reportUnhandledFiberErrors =
      Option(System.getenv("CATS_EFFECT_REPORT_UNHANDLED_FIBER_ERRORS"))
        .flatMap(x => Try(x.toBoolean).toOption)
    ...
    val cpuStarvationCheckThreshold =
      Option(System.getenv("CATS_EFFECT_CPU_STARVATION_CHECK_THRESHOLD"))
