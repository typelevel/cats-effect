...
private class LocalQueue extends LocalQueue.Padding {
   ...
   */
  private[this] val overflow = new FiberArrayList(BatchLength)
  ...
  def enqueue(fiber: IOFiber[_], external: ConcurrentLinkedQueue[IOFiber[_]]): Unit = {
    ...
    while (true) {
      ...
      val hd = Unsafe.getIntVolatile(this, headOffset)
      ...
      if (unsignedShortSubtraction(tl, steal) < LocalQueueCapacity) {
        ...
        val offset = FiberArrayBaseOffset + (tl & CapacityMask)
        Unsafe.putObject(buffer, offset, fiber)
        val newTl = unsignedShortAddition(tl, ReferencePointerSize)
        Unsafe.putOrderedInt(this, tailOffset, newTl)
        return
      }
      ...
      if (Unsafe.compareAndSwapInt(this, headOffset, hd, newHd)) {
        ...
        while (i < HalfLocalQueueCapacity) {
          val offset = FiberArrayBaseOffset + ((real + i) & CapacityMask)
          val fiber = Unsafe.getObject(buffer, offset).asInstanceOf[IOFiber[_]]
        ...
        // Reset the overflow list before next use.
        overflow.reset()
  ...
  def dequeue(): IOFiber[_] = {
    ...
    while (true) {
      ...
      val newReal = unsignedShortAddition(real, ReferencePointerSize)
      ...
      if (Unsafe.compareAndSwapInt(this, headOffset, hd, newHd)) {
        ...
        val offset = FiberArrayBaseOffset + (real & CapacityMask)
        val fiber = Unsafe.getObject(buffer, offset).asInstanceOf[IOFiber[_]]
        Unsafe.putObject(buffer, offset, null)
        return fiber
      }
  ...
  def steal(dst: LocalQueue): IOFiber[_] = {
    ...
    while (true) {
      ...
      var hd = Unsafe.getIntVolatile(this, headOffset)
      ...
      val tl = Unsafe.getIntVolatile(this, tailOffset)
      ...
      val actual = n >>> ReferencePointerShift
      val newActual = actual - (actual >>> 1)
      n = newActual << ReferencePointerShift
      ...
      if (Unsafe.compareAndSwapInt(this, headOffset, hd, newHd)) {
        ...
        var i = 0
        while (i < n) {
          val srcOffset = FiberArrayBaseOffset + ((steal + i) & CapacityMask)
          val dstOffset = FiberArrayBaseOffset + ((dstTl + i) & CapacityMask)
          val fiber = Unsafe.getObject(buffer, srcOffset)
          Unsafe.putObject(buffer, srcOffset, null)
          Unsafe.putObject(dst.buffer, dstOffset, fiber)
          i += ReferencePointerSize
        }
        ...
        while (true) {
          ...
          if (Unsafe.compareAndSwapInt(this, headOffset, hd, newHd)) {
            ...
            // `WorkerThread`.
            n -= ReferencePointerSize
            val newDstTl = unsignedShortAddition(dstTl, n)
            val offset = FiberArrayBaseOffset + (newDstTl & CapacityMask)
            val fiber = Unsafe.getObject(dst.buffer, offset).asInstanceOf[IOFiber[_]]
            Unsafe.putObject(dst.buffer, offset, null)
            ...
            // it again and retry.
            hd = Unsafe.getIntVolatile(this, headOffset)
            newReal = lsb(hd)
          }
        }
      }
  ...
  def drain(list: FiberArrayList): Unit = {
    ...
    while (true) {
      ...
      // Calculate the new "real" value of the head.
      val newReal = unsignedShortAddition(real, n)
      // Keep the old "steal" value, to signal that
      val newHd = if (steal == real) pack(newReal, newReal) else pack(steal, newReal)
