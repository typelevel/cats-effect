...
class ResourceSuite extends BaseScalaCheckSuite with DisciplineSuite {
...

  {
    ...
    // issue #4489 repro (1)
    real("Concurrent[Resource] - timeout finalizer (start/release race)") {
    ...
    // issue #4489 repro (2)
    real("Concurrent[Resource] - racePair finalizer (start/release race)") {
      val test: IO[Unit] = IO.ref(false).flatMap { ref =>
        val res = Resource.make(ref.set(true))(_ => ref.set(false))
    ...
    real("Concurrent[Resource] - racePair finalizer (start/cancel race)") {
      val test: IO[Unit] = IO.ref(false).flatMap { ref =>
        val res = Resource.make(ref.set(true))(_ => ref.set(false))
    ...
    // issue #4489 repro (3)
    real("Concurrent[Resource] - racePair finalizer variant (start/release race)") {
      val test: IO[Unit] = IO.ref(false).flatMap { ref =>
    ...
    real("Concurrent[Resource] - propagate successful result from a completed effect") {
      Resource
    ...
    real("Concurrent[Resource] - propagate error from a completed effect") {
      Resource
        .catsEffectTemporalForResource[IO]
        .sleep(50.millis)
        ...
        .attempt
        .use {
          case Left(error) => IO(assert(error.getClass == classOf[RuntimeException]))
          case Right(value) => IO(fail(s"Expected an error, got $value"))
        }
    }

    // additional #4059 test for Resource
    real("Concurrent[Resource] - timeout finalizer (#4059)") {
      val test = IO.ref(false).flatMap { ref =>
        ...
        program
          .attempt
          .flatMap(_ => ref.get.ifM(IO.raiseError(new Exception("not released")), IO.unit))
      }
