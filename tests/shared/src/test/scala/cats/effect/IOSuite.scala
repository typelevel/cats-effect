...
class IOSuite extends BaseScalaCheckSuite with DisciplineSuite with IOPlatformSuite {
  ...
  real("parTraverseN - not run more than `n` tasks at a time") {
    def task(counter: Ref[IO, Int], maximum: Ref[IO, Int]): IO[Unit] = {
      val acq = counter.updateAndGet { count => count + 1 }.flatMap { count =>
  ...
  ticked("parTraverseN - work for non-empty traverse") { implicit ticker =>
  ...
  real("parTraverseN - work for non-empty traverse on the runtime") {
    for {
      _ <- List(1)
        .parTraverseN(4)(i => IO.pure(i.toString))
        .map(r => assertEquals(r, List("1")))
      _ <- List(1, 2)
        .parTraverseN(3)(i => IO.pure(i.toString))
        .map(r => assertEquals(r, List("1", "2")))
      _ <- List(1, 2, 3)
        .parTraverseN(2)(i => IO.pure(i.toString))
        .map(r => assertEquals(r, List("1", "2", "3")))
      _ <- List(1, 2, 3, 4)
        .parTraverseN(1)(i => IO.pure(i.toString))
        .map(r => assertEquals(r, List("1", "2", "3", "4")))
      _ <- (1 to 10000)
        .toList
        .parTraverseN(2)(i => IO.pure(i.toString))
        .map(r => assertEquals(r, (1 to 10000).map(_.toString).toList))
    } yield ()
  }

  real("parTraverseN - be null-safe") {
    List[String]("a", "b", null, "d", null)
      .parTraverseN(2) {
        case "a" => IO.pure(null)
        case "b" => IO.pure("x")
        case "d" => IO.pure(null)
      ...
      .map(r => assertEquals(r, List(null, "x", "z", null, "z")))
  }

  ticked("parTraverseN - run finalizers in parallel") { implicit ticker =>
    // The first two effects are canceled while the third errors. The error must win.
    val test = for {
  ...
  ticked("parTraverseN_ - work for non-empty traverse") { implicit ticker =>
  ...
  real("parTraverseN_ - work for non-empty traverse on the runtime") {
    for {
      _ <- List(1).parTraverseN_(4)(i => IO.pure(i.toString))
      _ <- List(1, 2).parTraverseN_(3)(i => IO.pure(i.toString))
      _ <- List(1, 2, 3).parTraverseN_(2)(i => IO.pure(i.toString))
      _ <- List(1, 2, 3, 4).parTraverseN_(1)(i => IO.pure(i.toString))
      _ <- (1 to 10000).toList.parTraverseN_(2)(i => IO.pure(i.toString))
    } yield ()
  }
