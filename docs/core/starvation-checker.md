---
id: starvation-checker
title: Starvation Checker
---
...
In particular, it is very common to attempt to allocate only a single CPU (or less!), to an application. This in turn means that the application *itself* has very too few compute resources to schedule across, which creates bottlenecks. Effectively, this forces the underlying system (which is to say, the operating system kernel) to be the *only* form of scheduling, making systems like Cats Effect much less relevant.
