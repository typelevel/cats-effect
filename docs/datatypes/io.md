...
Returned is a [Fiber](../datatypes/fiber.md). You can think of fibers as being
...
whereas the second version gives you a [Fiber](../datatypes/fiber.md), letting
...
- One that takes a [ContextShift](../datatypes/contextshift.md) that manages the thread-pool used to trigger async boundaries.
...
but only inside an implementation of [IOApp](../datatypes/ioapp.md).
...
(see [ContextShift](../datatypes/contextshift.md)). You have a `ContextShift` in scope if:

1. via usage of [IOApp](../datatypes/ioapp.md) that gives you a `ContextShift` by default
