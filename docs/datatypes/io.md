...
`IO` implements all the typeclasses shown in the [hierarchy](../typeclasses/overview). Therefore
...
`Async#async` (see [Async](../typeclasses/async)) and can
...
Returned is a [Fiber](fiber). You can think of fibers as being
...
the laws of [ConcurrentEffect](../typeclasses/concurrent-effect).
...
whereas the second version gives you a [Fiber](fiber), letting
...
- One that takes a [ContextShift](contextshift) that manages the thread-pool used to trigger async boundaries.
...
but only inside an implementation of [IOApp](ioapp).
...
(see [ContextShift](contextshift)). You have a `ContextShift` in scope if:

1. via usage of [IOApp](ioapp) that gives you a `ContextShift` by default
