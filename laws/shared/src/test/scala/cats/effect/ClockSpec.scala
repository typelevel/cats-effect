...
class ClockSpec extends Specification with Discipline with ScalaCheck {
  ...
  implicit def execReaderWriterStateT(sbool: ReaderWriterStateT[FreeEitherSync, MiniInt, Int, MiniInt, Boolean]): Prop =
    run(sbool.runF).fold(
      Prop.exception(_),
      f => {
  ...
  checkAll("ContT[FreeEitherSync, Int, *]", ClockTests[ContT[FreeEitherSync, Int, *]].clock[Int, Int, Int])
  checkAll(
    "ReaderWriterStateT[FreeEitherSync, MiniInt, Int, MiniInt, *]",
    ClockTests[ReaderWriterStateT[FreeEitherSync, MiniInt, Int, MiniInt, *]].clock[Int, Int, Int]
  )
}
