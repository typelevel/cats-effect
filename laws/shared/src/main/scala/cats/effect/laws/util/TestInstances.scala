...
trait TestInstances {
  ...
  implicit def eqIO[A](implicit A: Eq[A], ec: TestContext): Eq[IO[A]] =
    (x: IO[A], y: IO[A]) => eqFuture[A].eqv(x.unsafeToFuture(), y.unsafeToFuture())
  ...
  implicit def eqIOPar[A](implicit A: Eq[A], ec: TestContext): Eq[IO.Par[A]] = {
      ...
      (x: IO.Par[A], y: IO.Par[A]) => eqFuture[A].eqv(unwrap(x).unsafeToFuture(), unwrap(y).unsafeToFuture())
    }
