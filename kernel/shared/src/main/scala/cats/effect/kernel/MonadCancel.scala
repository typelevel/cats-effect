 ...
 *   2. Cancelation can be masked via [[MonadCancel!.uncancelable]]. Masking
 *      is discussed in the next section.
 *   3. [[GenSpawn]] introduces external cancelation, another cancelation
...
object MonadCancel {
  ...
  private[kernel] trait IorTMonadCancel[F[_], L, E] extends MonadCancel[IorT[F, L, *], E] {
    ...
    override protected def transferControlLayer[A](fa: IorT[F, L, A])(
        f: IorT[F, L, A] => IorT[F, L, Unit]): IorT[F, L, A] =
      IorT(fa.value.flatMap(ia => f(IorT.fromIor(ia)).value.as(ia)))
