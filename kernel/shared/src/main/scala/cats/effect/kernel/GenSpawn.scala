...
trait GenSpawn[F[_], E] extends MonadCancel[F, E] with Unique[F] {
   ...
   *
   * This function is primarily useful when performing long-running computation that is outside of
   * the monadic context. For example:
   *
   * {{{
   *   fa.map(data => expensiveWork(data))
   * }}}
   *
   * In the above, we're assuming that `expensiveWork` is a function which is entirely compute-bound
   * but very long-running. A good rule of thumb is to consider a function "expensive" when its
   * runtime is around three or more orders of magnitude higher than the overhead of the `map`
   * function itself (which runs in around 5 nanoseconds on modern hardware). Thus, any
   * `expensiveWork` function which requires around 10 microseconds or longer to execute should be
   * considered "long-running".
   *
   * The danger is that these types of long-running actions outside of the monadic context can
   * result in degraded fairness properties. The solution is to add an explicit `cede` both before
   * and after the expensive operation:
   *
   * {{{
   ...
   * even when used with `cede`. This problem is somewhat fundamental to the nature of scheduling
   * such computation on carrier threads. Whenever possible, it is best to break apart any such
   * functions into multiple pieces invoked independently (e.g. via chained `map` calls) whenever
   * the execution time exceeds five or six orders of magnitude beyond the overhead of `map` itself
   * (around 1 millisecond on most hardware).
   *
